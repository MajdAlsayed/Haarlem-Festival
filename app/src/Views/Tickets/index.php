...
$pageTitle = 'Tickets — ' . ($app['site_name'] ?? 'Haarlem Festival');
$pageStyles = ['/css/pages/tickets.css'];
...
<main class="tickets-main">
    <section class="tickets-hero container">
        ...
        <?php if (!empty($cartFlashError)): ?>
            <p class="tickets-flash-error"><?= $h($cartFlashError) ?></p>
        <?php endif; ?>
                ...
                <?php foreach ($dayPasses as $p): ?>
                    <article class="tickets-card tickets-card--pass">
                        ...
                        <?php elseif (is_array($pst) && !empty($pst['low_stock'])): ?>
                            <p class="tickets-stock-badge tickets-stock-badge--low">
                                Only <?= $h((string) (int) ($pst['remaining'] ?? 0)) ?> left
                            </p>
                        <?php endif; ?>
                        ...
                        <div class="tickets-card-meta">
                            <?php if (($p['pass_day'] ?? '') !== ''): ?>
                                <div>
                                    <span class="tickets-meta-label">Day</span>
                                    <span class="tickets-meta-value"><?= $h($dayLabel((string) $p['pass_day'])) ?></span>
                                </div>
                            <?php endif; ?>

                            <?php if (($p['pass_time'] ?? '') !== ''): ?>
                                <div>
                                    <span class="tickets-meta-label">Time</span>
                                    <span class="tickets-meta-value"><?= $h((string) $p['pass_time']) ?></span>
                                </div>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="tickets-card-footer">
                            <span class="tickets-price">
                                <?= !empty($p['is_free']) ? 'FREE' : '€' . $h(number_format((float) $p['price'], 0)) ?>
                            </span>
                            ...
                            <?php if (empty($p['is_free']) && empty(($p['stock']['sold_out'] ?? false))): ?>
                                <form method="post" action="/cart/add" class="tickets-buy-form">
                                    ...
                                    <button type="submit" class="btn btn--primary btn--sm tickets-btn-buy">BUY ›</button>
                                </form>
                            <?php elseif (empty($p['is_free'])): ?>
                                <span class="btn btn--light btn--sm tickets-btn-buy tickets-btn-buy--disabled" aria-disabled="true">Sold out</span>
                            <?php else: ?>
                                <span class="btn btn--light btn--sm tickets-btn-buy tickets-btn-buy--disabled" aria-disabled="true">FREE</span>
                            <?php endif; ?>
                        </div>
                    </article>
                <?php endforeach; ?>
                ...
                <?php foreach ($items as $e):
                    ...
                    <article class="tickets-card">
                        ...
                        <?php elseif (is_array($est) && !empty($est['low_stock'])): ?>
                            <p class="tickets-stock-badge tickets-stock-badge--low">
                                Only <?= $h((string) (int) ($est['remaining'] ?? 0)) ?> left
                            </p>
                        <?php endif; ?>
                        ...
                        <div class="tickets-card-meta">
                            <div>
                                <span class="tickets-meta-label">Day</span>
                                <span class="tickets-meta-value"><?= $h($dayLabel((string) ($e['event_day'] ?? ''))) ?></span>
                            </div>

                            <div>
                                <span class="tickets-meta-label">Time</span>
                                <span class="tickets-meta-value"><?= $h($timeStr) ?></span>
                            </div>
                        </div>

                        <div class="tickets-card-footer">
                            <span class="tickets-price">
                                <?= !empty($e['is_free']) ? 'FREE' : '€' . $h(number_format((float) $e['price'], fmod((float) $e['price'], 1) > 0 ? 2 : 0)) ?>
                            </span>
