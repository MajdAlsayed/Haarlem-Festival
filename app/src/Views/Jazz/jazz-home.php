...
$hero = '/images/jazz/hero-jazz.jpg';

$eventCardImages = $vm->eventCardImages;
$allEventsOrder = $vm->allEventsOrder;

// Sort events to match All Events prototype order (row 1–4)
$eventsByIdx = [];
...
$events = [];
foreach ($allEventsOrder as $title) {
    if (isset($eventsByIdx[$title])) {
        $events[] = $eventsByIdx[$title];
    }
}
foreach ($eventsByIdx as $title => $e) {
    if (!in_array($title, $allEventsOrder, true) && strtolower($title) !== 'jazz evening') {
        $events[] = $e;
    }
}

// Only these detail pages exist now
$allowedDetails = [
    'gumbo-kings' => '/jazz/gumbo-kings',
    'karsu' => '/jazz/karsu',
    'gare-du-nord' => '/jazz/gare-du-nord',
];
...
<main>
    ...
    <section class="container jazz-intro">
        ...
        <div class="jazz-events-grid" id="jazzGrid">
            ...
            ?>
            <article class="jazz-event-card" data-day="<?= $h($day) ?>" data-event-id="<?= (int)$e['event_id'] ?>">
                <div class="jazz-event-media">
                    <img src="<?= $h($img) ?>"
                         alt="<?= $h($title) ?>">

                    <button type="button"
                            class="jazz-heart-btn"
                ...
                <div class="jazz-event-body">
                    ...
                    <!-- Artist details + Save to program (all cards) -->
                    <div class="jazz-event-actions">
                        ...
                        <?php endif; ?>
                        <button class="jazz-btn jazz-btn-secondary" type="button" data-save-event="<?= (int)$e['event_id'] ?>">Save to program</button>
                    </div>
                </div>
            </article>
            <?php endforeach; ?>
