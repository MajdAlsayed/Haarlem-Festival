...
$venueHero = $vm->getVenueHeroImage();

$slug     = strtolower(trim((string)($venue['slug'] ?? '')));
...
$kweekIntroText  = "Kweekcafé blends greenery, creativity, and community spirit. Surrounded by plants and natural light, visitors can relax while listening to heartfelt stories told by Haarlemmers from all walks of life. It is a peaceful, inspiring spot where storytelling feels authentic and close to nature.";
...
$kweekLeftBlocks = [
    ...
    [
        ...
        'text'  => "During Stories in Haarlem, KweekCafé transforms into an intimate open-air storytelling stage. Surrounded by garden paths, greenery, and Haarlem's iconic giant chessboard, visitors listen to tales that blend humor, tradition, and personal memories.",
    ],
];
...
<head>
    ...
    <title><?= h($venueName) ?> — Stories in Haarlem</title>
    <link rel="stylesheet" href="/css/style.css?v=<?= h($app['css_version'] ?? '1') ?>">
    <!-- details.css provides the canonical .stories-breadcrumb styles -->
   ...
   <nav class="stories-breadcrumb" aria-label="Breadcrumb">
    ...
    <?php if ($slugSafe === 'de-schuur'): ?>
        ...
        <section class="venue-hero-x"
                 style="background-image: linear-gradient(120deg, rgba(0,0,0,0.38), rgba(0,0,0,0.62)), url('<?= h($venueHero) ?>');">
        ...
        <section class="wrap-1150">
            ...
            <div class="split">
                ...
                <div class="split-img">
                    <img src="<?= h($schuurSplitImage) ?>" alt="De Schuur interior">
                </div>
            </div>
            ...
            <div class="days">
                <?php foreach (['all' => 'All Program', 'thursday' => 'Thursday', 'friday' => 'Friday', 'saturday' => 'Saturday'] as $key => $label): ?>
                    <a class="day <?= $vm->isActive($key) ? 'active' : '' ?>"
                       href="/stories/venue?slug=<?= h($slug) ?>&day=<?= h($key) ?>">
                        <?= h($label) ?>
                    </a>
                <?php endforeach; ?>
            </div>
            ...
            <div class="events">
                <?php if (empty($listToShow)): ?>
                    <p class="events-empty">
                        No events found for this venue<?= $selectedDay !== 'all' ? ' on ' . h($selectedDay) : '' ?>.
                    </p>
                <?php else: ?>
            ...
            <p class="section-intro"><?= h($schuurWhatIntro) ?></p>

            <div class="expect">
                <img src="<?= h($schuurExpectImg) ?>" alt="De Schuur atmosphere">
            ...
            <!-- Explore more cards -->
            <div class="more-wrap">
                <h2 class="section-title">Events to Explore</h2>
                            ...
                            <div class="stories-card-body">
                                ...
                                <div class="stories-card-meta">
                                    <div class="meta-row">
                                        ...
                                        <span><?= h(ucfirst($e['event_day'] ?? '')) ?> <?= h($eventTime($e)) ?></span>
                                    </div>
                                    <div class="meta-row">
                                        <span class="meta-ico">📍</span>
    ...
    <?php elseif ($slugSafe === 'kweekcafe'): ?>
        ...
        <section class="wrap-1150">
            ...
            <div class="split">
                ...
                <div class="split-img">
                    <img src="<?= h($kweekSplitImage) ?>" alt="Kweekcafé garden">
                </div>
            </div>
            ...
            <h2 class="section-title">Select a Day to Attend a Performance</h2>
            <div class="days">
                <?php foreach (['all' => 'All Program', 'thursday' => 'Thursday', 'friday' => 'Friday'] as $key => $label): ?>
                    <a class="day <?= $vm->isActive($key) ? 'active' : '' ?>"
                       href="/stories/venue?slug=<?= h($slug) ?>&day=<?= h($key) ?>">
                        <?= h($label) ?>
                    </a>
            ...
            <div class="kweek-inside-grid">
                <div class="kweek-inside-card">
                    <img src="<?= h($kweekInsideImg1) ?>" alt="Inside Kweekcafé 1">
                ...
                <div class="kweek-inside-card">
                    <img src="<?= h($kweekInsideImg2) ?>" alt="Inside Kweekcafé 2">
                    <div class="kweek-inside-text">
    ...
    <!-- ================================================================
         FALLBACK — any other venue slug
    ================================================================ -->
    <?php else: ?>
        <div class="wrap-1150 venue-not-found">
            <h1><?= h($venueName) ?></h1>
            <p>This venue page is coming soon.</p>
        </div>
    <?php endif; ?>
