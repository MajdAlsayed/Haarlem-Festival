...
<main class="admin-main">
    <div class="admin-container admin-container--wide">
        ...
        <form method="post" action="/admin/jazz/discography/save" class="admin-form admin-form--wide">
            ...
            <div class="admin-field">
                <label for="image_file">Cover image file</label>
                <input type="text" id="image_file" name="image_file" value="<?= $h((string) ($t['image_file'] ?? '')) ?>" class="admin-input" required>
                <small class="admin-hint">Filename in <code>public/images/jazz/</code></small>
            </div>

            <div class="admin-field">
                <label for="audio_file">Audio file path</label>
                <input type="text" id="audio_file" name="audio_file" value="<?= $h((string) ($t['audio_file'] ?? '')) ?>" class="admin-input" required>
                <small class="admin-hint">Under <code>public/audio/</code>, e.g. <code>Jazz audio/track.mp3</code></small>
            </div>

            <div class="admin-grid-2">
                <div class="admin-field">
                    <label for="release_year">Release year</label>
                    <input type="number" id="release_year" name="release_year" value="<?= $t['release_year'] !== null && $t['release_year'] !== '' ? $h((string) $t['release_year']) : '' ?>" class="admin-input">
                </div>
                <div class="admin-field">
                    <label for="duration_seconds">Duration (seconds)</label>
                    <input type="number" id="duration_seconds" name="duration_seconds" value="<?= $t['duration_seconds'] !== null && $t['duration_seconds'] !== '' ? $h((string) $t['duration_seconds']) : '' ?>" class="admin-input">
                </div>
            </div>
